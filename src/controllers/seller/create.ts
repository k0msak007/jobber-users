import { BadRequestError, ISellerDocument } from '@k0msak007/jobber-shared';
...
const seller = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    ...
    if (error?.details) {
      return next(new BadRequestError(error.details[0].message, 'Create seller() method error'));
    }

    const checkIfSellerExist: ISellerDocument | null = await getSellerByEmail(req.body.email);
    if (checkIfSellerExist) {
      return next(new BadRequestError('Seller already exist. Go to your account page to update.', 'Create seller() method error'));
    }
